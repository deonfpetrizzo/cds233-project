...
# ─────────────────────────────────────────────────────────────────────────────
# 2. CLF nominal controller (replaces the PID)
# ─────────────────────────────────────────────────────────────────────────────
# Planning model is the 2D double integrator  p_ddot = a  (input = acceleration;
# map_accel_to_thrusts realizes it via the attitude loop). The output y = p - p_des
# has relative degree 2, so a position-only V has L_g V = 0 and is unusable. As in
# Lecture 24/26 we build the CLF on the OUTPUT DYNAMICS (output + its derivative):
#
#     eta = [ p - p_des , v ]    eta_dot = A eta + B a    (per axis; x,y decouple)
#     A = [[0,1],[0,0]]          B = [0,1]^T
#     V(eta) = eta^T P eta,   P solves the CARE  A^T P + P A - P B R^-1 B^T P + Q = 0
#
# This is a valid (RES-)CLF: along the optimal feedback a = -K eta (K = R^-1 B^T P),
# V_dot = -eta^T Q eta <= -(lambda_min(Q)/lambda_max(P)) V.
#
# For a DENSE SWARM the CBF must coordinate many agents, so the nominal field needs
# to be SMOOTH. The pure min-norm CLF-QP (cost ||a||^2) has transient spikes where
# L_g V vanishes, which disrupt that coordination. We therefore use the lecture's
# robotic-CLF-QP structure: minimize deviation from the CLF-optimal feedback a_ff,
# subject to the (soft / relaxed) CLF decrease constraint:
#
#     min_{a,delta}  1/2 ||a - a_ff||^2 + 1/2 p_relax delta^2
#     s.t.   L_f V + L_g V a  <=  -clf_rate V + delta,     a_ff = -K eta
#
# Setting `use_feedforward=False` (a_ff = 0) recovers the pure min-norm CLF-QP.
def _build_clf(q_pos=9.0, q_vel=6.0, r_ctrl=1.0):
    A = np.array([[0.0, 1.0], [0.0, 0.0]])
    B = np.array([[0.0], [1.0]])
    Q = np.diag([q_pos, q_vel])
    R = np.array([[r_ctrl]])
    P = solve_continuous_are(A, B, Q, R)
    M = A.T @ P + P @ A                          # used in  L_f V = eta^T M eta
    PB = (P @ B).flatten()                       # used in  L_g V = 2 eta^T (P B)
    K = (np.linalg.solve(R, B.T @ P)).flatten()  # CLF-optimal (LQR) gain [k1, k2]
    lam = float(np.min(np.linalg.eigvalsh(Q)) / np.max(np.linalg.eigvalsh(P)))
    return P, M, PB, K, lam

_P, _M, _PB, _K, _LAM_GUARANTEED = _build_clf()

def solve_clf_qp(state, target_pos, clf_rate=None, p_relax=1.0e4,
                 a_max=None, use_feedforward=True):
    """
    Per-agent CLF-QP nominal controller. Regulates (x, y) -> (x_des, y_des) with
    v -> 0 as a SOFT constraint, so the downstream CBF-QP can override it for safety.
    Single-constraint closed-form (KKT) solution; returns a 2D acceleration [ax, ay].
    """
    if clf_rate is None:
        clf_rate = _LAM_GUARANTEED
    ...
    a_ff = np.array([-_K @ eta_x, -_K @ eta_y]) if use_feedforward else np.zeros(2)

    V   = eta_x @ _P @ eta_x + eta_y @ _P @ eta_y
    LfV = eta_x @ _M @ eta_x + eta_y @ _M @ eta_y
    LgV = np.array([2.0 * eta_x @ _PB, 2.0 * eta_y @ _PB])

    # CLF margin of the feedforward; if satisfied, return a_ff (smooth). Else relax.
    slack = (LfV + clf_rate * V) + LgV @ a_ff
    if slack > 0.0:
        mu = slack / (LgV @ LgV + 1.0 / p_relax)
        a = a_ff - mu * LgV
    else:
        a = a_ff
...
# ─────────────────────────────────────────────────────────────────────────────
#   CBF-QP  (UNMODIFIED — collision-avoidance safety filter)
# ─────────────────────────────────────────────────────────────────────────────
...
class MultiQuadrotorSim:
    ...
    def run_centralized(self, t_span=(0.0, 8.0), dt=0.04,
                        clf_rate=None, p_relax=1.0e4, a_max=None, use_feedforward=True):
        ...
        print(f"CLF nominal: K={np.round(_K,2)}, guaranteed rate "
              f"lambda_min(Q)/lambda_max(P)={_LAM_GUARANTEED:.3f}")
        ...
        for k in range(1, len(time_grid)):
            ...
            # Nominal control from the CLF-QP (replaces the old PID)
            u_nom = np.zeros((N, 2))
            for i, inst in enumerate(self.instances):
                u_nom[i] = solve_clf_qp(current_states[i], inst.target,
                                        clf_rate=clf_rate, p_relax=p_relax,
                                        a_max=a_max, use_feedforward=use_feedforward)
...
def run_drone_show_scenario():
    ...
    # Run the simulation! (CLF-QP nominal + unmodified CBF-QP safety filter)
